...
app.fn = (function() {
  ...
  var _ROUTE_FETCH_URL = _HOST_ADDRESS + '/webapp/14850/www/fetchPlanner.php';
  var _ADDON_URL = _HOST_ADDRESS + '/webapp/14850/www/addon.php';
  var _ROUTE_SEARCH_TIMEOUT = 1500;
  ...
  var _showDestination = function(lat2, lng2) {
    // console.log('pan to: ' + lat2 + ',' + lng2);
    if(app.lastMarker) {
      app.lastMarker.setMap(null);
    }
    // console.log(app.map);
    ...
    setTimeout(function() {
      app.map.panTo(new google.maps.LatLng(app.mapLat, app.mapLng));
    }, 3000);
    setTimeout(function() {
      google.maps.event.trigger(app.map, 'resize');
    }, 4000);
  ...
  var _bind_route_action = function() {
    ...
    $('.result-box').click(function() {
      ...
      $('.route-box').remove();
      $('#status').html('Searching TCAT Website');
      $('#route').css({'display': 'none', 'z-index': 2000});
      $('#route').fadeIn();
  ...
  var _displayRoutes = function() {
    ...
    // Select top 30 and eliminate 1.5x && 20 min longer than shortest time, or take longer than 3 hours
    _arrivedRoutes = _arrivedRoutes.slice(0, 50);
    var minTimeTotal = _arrivedRoutes[0]['timeTotal'];
    var maxTimeTotal = Math.max.apply(null, [1.5 * minTimeTotal, minTimeTotal + 20]);
  ...
  return {
    ...
    addRoutes: function(routes, token) {
      ...
      // if(currentTime - _firstArrivalTime > _ROUTE_SEARCH_TIMEOUT) {
        // _displayRoutes();
        // _token = (new Date()).getTime();
      // }
    },
  ...
  // Show destination on map
    // showDestination: function(lat2, lng2) {
      // console.log('pan to: ' + lat2 + ',' + lng2);
      // if(_lastMarker) {
        // _lastMarker.setMap(null);
      // }
      // console.log(app.map);
      // app.map.panTo(new google.maps.LatLng(lat2, lng2));
      // var marker = new google.maps.Marker({
        // position: new google.maps.LatLng(lat2, lng2)
      // });
      // marker.setMap(app.map);
