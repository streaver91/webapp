...
app.fn = (function() {
  ...
  // Route selection
  var _ROUTE_SEARCH_TIMEOUT = 10000;
  var _PENALTY_DISTANCE = 400;
  var _MAX_DISTANCE = 800;
  var _LOCATION_RETURN_MAX = 5;
  var _STOPS_SEARCH_RANGE = 4;
  var _STOPS_SEARCH_RANGE2 = _STOPS_SEARCH_RANGE * _STOPS_SEARCH_RANGE;
  ...
  var _places = [
    {'place': 'Regal Cinemas Ithaca Mall Stadium 14 Movie Theater', 'location': '42.484358,-76.491871'},
    {'place': 'Ithaca Bakery (Triphammer Mall)', 'location': '42.480050,-76.480844'},
    {'place': 'Ithaca Bakery (Downtown)', 'location': '42.442731,-76.508891'},
    {'place': 'Saigon Kitchen', 'location': '42.439418,-76.507448'},
    {'place': 'Walmart Pharmacy', 'location': '42.426892, -76.513832'},
    {'place': 'Maines Food & Party Warehouse', 'location': '42.424833,-76.515313'},
    {'place': 'Ithaca Commons', 'location': '42.439526,-76.497315'},
    {'place': 'Ithaca Tofu Oriental Market', 'location': '42.479482,-76.477792'},
    {'place': 'Chapter House Brew Pub', 'location': '42.442229,-76.489993'},
    {'place': 'Purity Ice Cream Company, Inc', 'location': '42.444452,-76.509097'}
  ];
  
  //  Temp variables
  var _lastAddress = null;
  var _arrivedRoutes = [];
  var _arrivedCount = 0;
  ...
  var _search = function() {
    ...
    $.get(_GEOCODING_URL, {address: address, key:_GOOGLE_KEY, components: 'postal_code:14850'}, function(data) {
      ...
      // Geocoding results
      if(results.length != 0 && results[0].formatted_address != 'Ithaca, NY 14850, USA') {
      ...
      for(var i = 0; i < _stops.length; i++) {
        ...
        if(address.length != 0 && currentStop['stop'].toUpperCase().indexOf(address.toUpperCase()) != -1) {
        ...
        if($('.search-results > .result-box').length >= _LOCATION_RETURN_MAX) {
          break;
        }
      }
      // Interested Places
      for(var i = 0; i < _places.length; i++) {
        ...
        if(address.length != 0 && currentPlace['place'].toUpperCase().indexOf(address.toUpperCase()) != -1) {
          $('<a class="result-box" href="javascript:;" data-location="' + currentPlace['location'] + '">' + currentPlace['place'] + '</a>').appendTo('#search-results');
        }
        if($('.search-results > .result-box').length >= _LOCATION_RETURN_MAX) {
          break;
        }
      ...
      if($('.search-results > .result-box').length == 0) {
        $('<div class="result-box" >No Result Found</div>').appendTo('#search-results');
      }
      _bind_route_action();
      
      $('#search-results').finish().slideDown();
  ...
  var _recommend = function() {
    ...
    $('#special-results').finish().slideDown();
  }
  ...
  var _searchHandler = function(event) {
    ...
    if(searchStr == '' && event.type == 'focusout') {
      $this.val(_HINTS[app.type]);
    }
  };
  
  var _searchFocusHandler = function() {
    var $this = $(this);
    if($this.val() == _HINTS[app.type]) {
      $this.val('');
    } else {
      $this.select();
    }
  ...
  var _load_recent = function() {
    ...
    if(recentLocations) {
      for(var i = 0; i < recentLocations.length; i++) {
        if(recentLocations[i].latlng == (_getQuery('lat2') + ',' + _getQuery('lng2')) && app.type == 'from') {
          continue;
        }
  ...
  var _route = function(lat1, lng1, lat2, lng2) {
    ...
    var stops1 = _getNearestStops(lat1, lng1, _STOPS_SEARCH_RANGE);
    var stops2 = _getNearestStops(lat2, lng2, _STOPS_SEARCH_RANGE);
  ...
  var _bind_route_action = function() {
    $('.result-box').click(function() {
      ...
      if(app.type == 'to') {
        location.href = 'from.html?lat2=' + locationArr[0] + '&lng2=' + locationArr[1] + '&addr=' + address;
      } else if(app.type == 'from') {
        var lat2 = _getQuery('lat2');
        var lng2 = _getQuery('lng2');
        location.href = 'route.html?lat1=' + locationArr[0] + '&lng1=' + locationArr[1] + '&lat2=' + lat2 + '&lng2=' + lng2;
      }
  ...
  var _displayRoutes = function() {
    ...
    // Select top 50 and eliminate 2x && 20 min longer than shortest time, or take longer than 3 hours
    ...
    for(var i = 0; i < _arrivedRoutes.length; i++) {
      ...
      var curTimeTotal = _arrivedRoutes[i]['timeTotal'];
      for(var j = _arrivedRoutes.length - 1; j > i; j--) {
        if(curSequence == _arrivedRoutes[j]['routeSequence'] && _arrivedRoutes[j]['timeTotal'] - curTimeTotal > 1) {
          _arrivedRoutes.splice(j, 1);
        }
      }
    }
  ...
  return {
    init: function(type) {
      ...
      if(type == 'from') {
        $('#destination').html('TO: ' + _getQuery('addr'));  
      }
      _load_recent();
      _bind_route_action();
    ...
    routeInit: function() {
      var lat1 = parseFloat(_getQuery('lat1'));
      var lng1 = parseFloat(_getQuery('lng1'));
    ...
    addRoutes: function(routes, token) {
      ...
      _arrivedCount++;
      $('#route-status > span').html('Retriving Routes: ' + Math.floor(_arrivedCount / _STOPS_SEARCH_RANGE2 * 100) + '%');
      $.merge(_arrivedRoutes, routes);
      if(_arrivedCount == _STOPS_SEARCH_RANGE2) {
        _displayRoutes();
      }
      if(_firstArrivalTime == 0) {
        ...
        setTimeout(function() {
          if(_arrivedCount < _STOPS_SEARCH_RANGE2) {
            // Not displayed yet
            console.log('Retriving timeout. Force display. Current Cnt: ' + _arrivedCount);
            _displayRoutes();
          }
